...
## Get model
gen = Generator(tstep=fstep, use_cuda=True)
gen.to(device)
dis = Discriminator(in_channel=in_c)
dis.to(device)

fake_input = torch.ones((batch_size, tstep, in_c, 256, 256)).to(device)
fake_label = torch.ones((batch_size, fstep, in_c, 256, 256)).to(device)
